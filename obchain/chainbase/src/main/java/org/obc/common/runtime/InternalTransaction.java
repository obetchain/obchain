...
public class InternalTransaction {
  ...
  /* the amount of obc to transfer (calculated as sun) */
  private long value;
   ...
   */
  public InternalTransaction(Transaction obc, InternalTransaction.obcType obcType)
      throws ContractValidateException {
    this.transaction = obc;
    TransactionCapsule obcCap = new TransactionCapsule(obc);
    this.protoEncoded = obcCap.getData();
    this.nonce = 0;
    ...
    this.deep = -1;
    if (obcType == obcType.obc_CONTRACT_CREATION_TYPE) {
      ...
      this.transferToAddress = WalletUtil.generateContractAddress(obc);
      this.note = "create";
    ...
    } else if (obcType == obcType.obc_CONTRACT_CALL_TYPE) {
    ...
    this.hash = obcCap.getTransactionId().getBytes();
  }
  ...
  public InternalTransaction(byte[] parentHash, int deep, int index,
    ...
    // in this case, value also can represent a tokenValue when tokenId is not null, otherwise it is a obc callvalue.
    this.value = value;
    this.data = ArrayUtils.nullToEmpty(data);
    this.nonce = nonce;
    this.hash = getHash();
    // in a contract call contract case, only one value should be used. obc or a token. can't be both. We should avoid using
  ...
  public enum obcType {
    obc_PRECOMPILED_TYPE,
    obc_CONTRACT_CREATION_TYPE,
    obc_CONTRACT_CALL_TYPE,
    obc_UNKNOWN_TYPE,
  }
