...
public class StorageMarket {
  ...
  private long exchangeToSupply(boolean isobc, long quant) {
    logger.info("isobc: " + isobc);
    long balance = isobc ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchangeToSupply2(boolean isobc, long quant) {
    logger.info("isobc: " + isobc);
    long balance = isobc ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  private long exchange_from_supply(boolean isobc, long supplyQuant) {
    long balance = isobc ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    supply -= supplyQuant;
  ...
  public long exchange(long from, boolean isobc) {
    long relay = exchangeToSupply(isobc, from);
    return exchange_from_supply(!isobc, relay);
  }
