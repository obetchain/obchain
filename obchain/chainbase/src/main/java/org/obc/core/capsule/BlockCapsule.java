...
import org.obc.core.capsule.utils.MerkleTree;
...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public BlockCapsule(long timestamp, ByteString parentHash, long number,
    ...
    // block
    Block.Builder blockBuild = Block.newBuilder();
    transactionList.forEach(obc -> blockBuild.addTransactions(obc));
    this.block = blockBuild.setBlockHeader(blockHeader).build();
    initTxs();
  }
  ...
  public void addTransaction(TransactionCapsule pendingobc) {
    this.block = this.block.toBuilder().addTransactions(pendingobc.getInstance()).build();
    getTransactions().add(pendingobc);
  }
  ...
  private void initTxs() {
    transactions = this.block.getTransactionsList().stream()
        .map(obc -> new TransactionCapsule(obc))
