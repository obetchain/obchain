...
if [[ "$TRAVIS_BRANCH" = "develop" || "$TRAVIS_BRANCH" = "master" ]];then
    ...
    stest_server=""
    docker_num_in_145=`ssh -p 22008 -t java-obc@47.93.42.145 'docker ps -a | wc -l'`
    docker_num_in_145=`echo $docker_num_in_145 | tr -d "\r"`
    docker_num_in_60=`ssh -p 22008 -t java-obc@47.93.18.60 'docker ps -a | wc -l'`
    ...
    ssh java-obc@$stest_server -p 22008 $change_branch_CMD
    `ssh java-obc@$stest_server -p 22008 sh /data/workspace/docker_workspace/do_stest.sh >$stestlogname 2>&1` &
