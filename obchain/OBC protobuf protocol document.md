...
This is the description of  Google Protobuf implementation of obc's protocol.
...
- obc has 3 `types` of account: Normal, AssetIssue, Contract
...
- message `Account` has multiple attributes and 2 nested messages: 
  ...
  `balance`: the obc balance of this account.

  `votes`: received votes of this account. – e.g. *{(“0x1b7w…9xj3”,323), (“0x8djq…j12m”,88),…,(“0x82nd…mx6i”,10001)}*.

  `asset`: other assets except obc in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*.

  `assetV2`: other assets except obc in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*. (used after allowing same name of token87)

  `frozen`: the freezed obc of this account for receiving bandwidth
...
- message `TransactionRet`
  ...
  - message `ResourceReceipt`
    ...
    `net_fee`: consume yourself obc of net usage.
  ...
  - Message `InternalTransaction`
    ...
    `callValueInfo`: Refers to asset transfer information in internal transactions, including obc and trc10.
  ...
  - message `TransactionInfo`
   ...
   `unfreeze_amount`: unfreeze obc amount.
...
- obc has 33 types of Contracts declared within [`Transaction`](#trans).
...
- There are 15 types of results while deploying contracts (refer to `Transaction.Result`):
  ...
  - message `VoteWitnessContract`
    ...
    - message `AssetIssueContract`
      ...
      `obc_num`: obc num defines token price.

      `precision`: precision.

      `num`: obc num defines token price.
    ...
    - message `FreezeBalanceContract`
      ...
      `frozen_balance`: frozen amount of obc.

      `frozen_duration`: frozen duration of obc.

      `resource`: type of resource gained from freezing obc.
    ...
    - message `UnfreezeBalanceContract`
     ...
     - message `TriggerSmartContract`
       ...
       `call_value`: obc amount sent to smart contract.
...
- message `SmartContract`
  ...
  `call_value`: amount of obc that send to the smart contract.
  ...
  `obc_hash`:  transactionId of Deploying contract transaction.
