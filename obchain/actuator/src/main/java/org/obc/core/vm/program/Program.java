...
import org.obc.common.utils.*;
import org.obc.core.capsule.*;
import org.obc.core.config.Parameter;
...
import org.obc.core.exception.obcException;
import org.obc.core.store.DynamicPropertiesStore;
import org.obc.core.utils.TransactionUtil;
import org.obc.core.vm.*;
import org.obc.core.vm.config.VMConfig;
import org.obc.core.vm.nativecontract.*;
import org.obc.core.vm.nativecontract.param.*;
import org.obc.core.vm.program.invoke.ProgramInvoke;
import org.obc.core.vm.program.invoke.ProgramInvokeFactory;
import org.obc.core.vm.program.invoke.ProgramInvokeFactoryImpl;
import org.obc.core.vm.program.listener.CompositeProgramListener;
import org.obc.core.vm.program.listener.ProgramListenerAware;
import org.obc.core.vm.program.listener.ProgramStorageChangeListener;
import org.obc.core.vm.repository.Repository;
import org.obc.core.vm.trace.ProgramTrace;
import org.obc.core.vm.trace.ProgramTraceListener;
...
import org.obc.protos.contract.SmartContractOuterClass.SmartContract.Builder;
...
import static org.obc.common.utils.ByteUtil.stripLeadingZeroes;
...
public class Program {
  ...
  /**
   * @param transferAddress the address send obc to.
   * @param value the obc value transferred in the internal transaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = TransactionTrace.convertToobcAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = TransactionTrace.convertToobcAddress(obtainerAddress.getLast20Bytes());
      ...
      try {
        ...
        if (VMConfig.allowTvmConstantinople()) {
          throw new TransferException(
              "transfer all token or transfer all obc failed in suicide: %s", e.getMessage());
        }
  ...
  private void suicideFreezeBalanceAndVote(byte[] owner, byte[] obtainer, Repository repository) {
    ...
    if (FastByteComparisons.compareTo(obtainer, 0, 20,
            TransactionTrace.convertToobcAddress(new byte[20]), 0, 20) == 0
      ...
      repository
              .addTotalNetWeight(-unfreezeBalance / Parameter.ChainConstant.obc_PRECISION);
    } else {
    ...
    //vote
    {
      ...
      if (!oldVotes.isEmpty()) {
        ...
        byte[] zeroAddress = TransactionTrace.convertToobcAddress(new byte[20]);
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    byte[] senderAddress = TransactionTrace
        .convertToobcAddress(this.getContractAddress().getLast20Bytes());
    ...
    if (VMConfig.allowTvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setobcHash(ByteString.copyFrom(rootTransactionId));
        }
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(
                TransactionTrace.convertToobcAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = TransactionTrace
        .convertToobcAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = TransactionTrace
        .convertToobcAddress(getContractAddress().getLast20Bytes());
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer obc failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
        deposit.addBalance(senderAddress, -endowment);
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(
            TransactionTrace.convertToobcAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit()
        .getCode(TransactionTrace.convertToobcAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] obcAddr = TransactionTrace.convertToobcAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(obcAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(obcAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(obcAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState()
        .getBalance(TransactionTrace.convertToobcAddress(address.getLast20Bytes()));
  ...
  public DataWord getRewardBalance(DataWord address) {
    ...
    long rewardBalance = contractService
            .queryReward(TransactionTrace.convertToobcAddress(address.getLast20Bytes()), getContractState());
    return new DataWord(rewardBalance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(TransactionTrace.convertToobcAddress(address.getLast20Bytes()));
  ...
  public DataWord isSRCandidate(DataWord address) {
    WitnessCapsule witnessCapsule = getContractState()
            .getWitnessCapsule(TransactionTrace.convertToobcAddress(address.getLast20Bytes()));
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState()
        .getTokenBalance(TransactionTrace.convertToobcAddress(address.getLast20Bytes()),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    ...
    if(VMConfig.allowTvmIstanbul()) {
      ...
      senderAddress = TransactionTrace
          .convertToobcAddress(this.getCallerAddress().getLast20Bytes());
    }
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(TransactionTrace.convertToobcAddress(msg.getType().callIsDelegate()
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowTvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer trc10 or obc to non-exist address(would create one)
  ...
  public boolean stake(DataWord srAddress, DataWord stakeAmount) {
    ...
    stakeParam.setSrAddress(TransactionTrace.convertToobcAddress(srAddress.getLast20Bytes()));
  ...
  public void withdrawReward() {
    ...
    byte[] ownerAddress = TransactionTrace.convertToobcAddress(getContractAddress().getLast20Bytes());
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(obcException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(obcException e) {
      return new PrecompiledContractException(e.getMessage());
    }
