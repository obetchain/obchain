...
import static org.obc.common.crypto.Hash.sha3omit12;
...
import org.obc.api.GrpcAPI.TransactionSignWeight.Result;
import org.obc.protos.Protocol.Permission;
import org.obc.protos.Protocol.Permission.PermissionType;
...
public class TransactionUtil {
  ...
  public static TransactionCapsule getTransactionSign(TransactionSign transactionSign) {
    ...
    TransactionCapsule obc = new TransactionCapsule(transactionSign.getTransaction());
    obc.sign(privateKey);
    return obc;
  }
  ...
  public TransactionCapsule addSign(TransactionSign transactionSign)
    ...
    byte[] privateKey = transactionSign.getPrivateKey().toByteArray();
    TransactionCapsule obc = new TransactionCapsule(transactionSign.getTransaction());
    obc.addSign(privateKey, chainBaseManager.getAccountStore());
    return obc;
  }

  public TransactionSignWeight getTransactionSignWeight(Transaction obc) {
    ...
    try {
      ...
      if (obc.getSignatureCount() > 0) {
        ...
        long currentWeight = TransactionCapsule.checkWeight(permission, obc.getSignatureList(),
            Sha256Hash.hash(CommonParameter.getInstance()
                .isECKeyCryptoEngine(), obc.getRawData().toByteArray()), approveList);
