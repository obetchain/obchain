...
import static org.obc.core.vm.utils.MUtil.transfer;
import static org.obc.core.vm.utils.MUtil.transferToken;
...
import org.obc.common.runtime.InternalTransaction.ExecutorType;
...
import org.obc.core.vm.EnergyCost;
import org.obc.core.vm.LogInfoTriggerParser;
import org.obc.core.vm.VM;
import org.obc.core.vm.VMConstant;
import org.obc.core.vm.VMUtils;
import org.obc.core.vm.config.ConfigLoader;
import org.obc.core.vm.config.VMConfig;
import org.obc.core.vm.program.Program;
import org.obc.core.vm.program.ProgramPrecompile;
...
import org.obc.core.vm.program.Program.TransferException;
...
public class VMActuator implements Actuator2 {
  ...
  @Getter
  @Setter
  private InternalTransaction.obcType obcType;
  ...
  public void validate(Object object) throws ContractValidateException {
    ...
    obc = context.getobcCap().getInstance();
    blockCap = context.getBlockCap();
    //Route Type
    ContractType contractType = this.obc.getRawData().getContract(0).getType();
    //Prepare Repository
    ...
    switch (contractType.getNumber()) {
      case ContractType.TriggerSmartContract_VALUE:
        obcType = obcType.obc_CONTRACT_CALL_TYPE;
        call();
        break;
      case ContractType.CreateSmartContract_VALUE:
        obcType = obcType.obc_CONTRACT_CREATION_TYPE;
        create();
        break;
      default:
    ...
    try {
      if (vm != null) {
        if (null != blockCap && blockCap.generatedByMyself && blockCap.hasWitnessSignature()
            && null != TransactionUtil.getContractRet(obc)
            && contractResult.OUT_OF_TIME == TransactionUtil.getContractRet(obc)) {
        ...
        if (isConstantCall) {
          long callValue = TransactionCapsule.getCallValue(obc.getRawData().getContract(0));
          long callTokenValue = TransactionUtil
              .getCallTokenValue(obc.getRawData().getContract(0));
        ...
        if (obcType.obc_CONTRACT_CREATION_TYPE == obcType && !result.isRevert()) {
  ...
  private void create()
    ...
    try {
      long feeLimit = obc.getRawData().getFeeLimit();
      ...
      rootInternalTransaction = new InternalTransaction(obc, obcType);
      ...
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(obcType.obc_CONTRACT_CREATION_TYPE, executorType, obc,
      ...
      byte[] txId = TransactionUtil.getTransactionId(obc).getBytes();
  ...
  private void call()
    ...
    TriggerSmartContract contract = ContractCapsule.getTriggerContractFromTransaction(obc);
    if (contract == null) {
      return;
    }
    ...
    if (isNotEmpty(code)) {
      ...
      long vmShouldEndInUs = vmStartInUs + thisTxCPULimitInUs;
      ProgramInvoke programInvoke = programInvokeFactory
          .createProgramInvoke(obcType.obc_CONTRACT_CALL_TYPE, executorType, obc,
              tokenValue, tokenId, blockCap.getInstance(), repository, vmStartInUs,
              vmShouldEndInUs, energyLimit);
      if (isConstantCall) {
  ...
  private double getCpuLimitInUsRatio() {
    ...
    if (ExecutorType.ET_NORMAL_TYPE == executorType) {
      ...
      if (this.blockCap != null && blockCap.generatedByMyself &&
        ...
        if (obc.getRet(0).getContractRet() == contractResult.OUT_OF_TIME) {
  ...
  public long getTotalEnergyLimitWithFixRatio(AccountCapsule creator, AccountCapsule caller,
    ...
    if (Arrays.equals(creator.getAddress().toByteArray(), caller.getAddress().toByteArray())) {
      ...
      // which will cost the feeLimit obc when the creator's frozen energy is 0.
