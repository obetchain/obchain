...
public class Storage {
  ...
  private static byte[] addrHash(byte[] address, byte[] obcHash) {
    if (ByteUtil.isNullOrZeroArray(obcHash)) {
      return Hash.sha3(address);
    }
    return Hash.sha3(ByteUtil.merge(address, obcHash));
  }

  public void generateAddrHash(byte[] obcId) {
    // update addreHash for create2
    addrHash = addrHash(address, obcId);
  }
