# obc protobuf protocol

## obc使用Google protobuf协议，协议内容涉及到账户，区块，传输多个层面。
...
+	更进一步的，基本账户可以申请成为验证节点，验证节点具有额外的属性，投票统计数目，公钥，URL，以及历史表现等参数。
   ...
   `balance`:该账户的obc余额——比如：_4213312_。  
   `votes`:账户所得投票数——比如：_{(“0x1b7w…9xj3”,323),(“0x8djq…j12m”,88),…,(“0x82nd…mx6i”,10001)}_。  
   `asset`：除obc以外账户上的其他资产——比如：_{<”WishToken”,66666>,<”Dogie”,233>}_。
...
+	传输涉及的协议Inventory主要用于传输中告知接收方传输数据的清单。
   ...
   `type`：清单类型——比如：_0_ 代表`obc`。  
   `ids`：清单中的物品ID。

   `InventoryType`包含`obc`和 `BLOCK`。  
   `obc`：交易。  
   `BLOCK`：区块。
   ...
   消息体 `Items`包含4种参数：  
   `type`：物品类型——比如：_1_ 代表 `obc`。  
   `blocks`：物品中区块。  
   `blockheaders`：区块头。  
   `transactions`：交易。

   `Items`有四种类型，分别是 `ERR`， `obc`，`BLOCK` 和`BLOCKHEADER`。  
   `ERR`：错误。  
   `obc`：交易。  
   `BLOCK`：区块。  
   `BLOCKHEADER`：区块头。
