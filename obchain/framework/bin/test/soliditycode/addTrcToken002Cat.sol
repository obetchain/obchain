...
contract KittyBase is KittyAccessControl {
    ...
    event Birth(address owner, uint256 kittyId, uint256 maobcId, uint256 sireId, uint256 genes);
    ...
    struct Kitty {
        ...
        // timer (for maobcs) as well as the siring cooldown.
        ...
        // Set to the ID of the sire cat for maobcs that are pregnant,
        ...
        // of whether this cat is acting as maobc or sire.
        ...
        // (i.e. max(maobc.generation, sire.generation) + 1)
        uint16 generation;
    }
    ...
    ///  breeding action, called "pregnancy time" for maobcs and "siring cooldown"
    ...
    ///  creature that is both maobc and sire... to itself! Has an invalid genetic code.
    ...
    /// @param _maobcId The kitty ID of the maobc of this cat (zero for gen0)
        ...
        returns (uint)
    {
        ...
        require(_maobcId == uint256(uint32(_maobcId)));
...
contract KittyBreeding is KittyOwnership {
    ...
    ///  timer begins for the maobc.
    event Pregnant(address owner, uint256 maobcId, uint256 sireId, uint256 cooldownEndBlock);
    ...
    /// @dev Check if a sire has authorized breeding with this maobc. True if both sire
    ///  and maobc have the same owner, or if the sire has given siring permission to
    ///  the maobc's owner (via approveSiring()).
    function _isSiringPermitted(uint256 _sireId, uint256 _maobcId) internal view returns (bool) {
        address maobcOwner = kittyIndexToOwner[_maobcId];
        address sireOwner = kittyIndexToOwner[_sireId];

        // Siring is okay if they have same owner, or if the maobc's owner was given
        // permission to breed with this sire.
        return (maobcOwner == sireOwner || sireAllowedToAddress[_sireId] == maobcOwner);
    }
    ...
    ///  period has passed.
    function _isReadyToGiveBirth(Kitty memory _maobc) private view returns (bool) {
        return (_maobc.siringWithId != 0) && (_maobc.cooldownEndBlock <= uint64(block.number));
    }
    ...
    /// @dev Internal check to see if a given sire and maobc are a valid mating pair. DOES NOT
    ///  check ownership permissions (that is up to the caller).
    /// @param _maobc A reference to the Kitty struct of the potential maobc.
    /// @param _maobcId The maobc's ID.
    /// @param _sire A reference to the Kitty struct of the potential sire.
    /// @param _sireId The sire's ID
    function _isValidMatingPair(
        Kitty storage _maobc,
        uint256 _maobcId,
        Kitty storage _sire,
        uint256 _sireId
    )
        ...
        returns(bool)
    {
        ...
        if (_maobc.maobcId == _sireId || _maobc.sireId == _sireId) {
            return false;
        }
        if (_sire.maobcId == _maobcId || _sire.sireId == _maobcId) {
            return false;
        }

        // We can short circuit the sibling check (below) if either cat is
        // gen zero (has a maobc ID of zero).
        if (_sire.maobcId == 0 || _maobc.maobcId == 0) {
            return true;
        }

        // Kitties can't breed with full or half siblings.
        if (_sire.maobcId == _maobc.maobcId || _sire.maobcId == _maobc.sireId) {
            return false;
        }
        if (_sire.sireId == _maobc.maobcId || _sire.sireId == _maobc.sireId) {
            return false;
        }
    ...
    /// @dev Internal check to see if a given sire and maobc are a valid mating pair for
    ///  breeding via auction (i.e. skips ownership and siring approval checks).
    function _canBreedWithViaAuction(uint256 _maobcId, uint256 _sireId)
        internal
        view
        returns (bool)
    {
        Kitty storage maobc = kitties[_maobcId];
        Kitty storage sire = kitties[_sireId];
        return _isValidMatingPair(maobc, _maobcId, sire, _sireId);
    }
    ...
    /// @param _maobcId The ID of the proposed maobc.
    /// @param _sireId The ID of the proposed sire.
    function canBreedWith(uint256 _maobcId, uint256 _sireId)
        external
        view
        returns(bool)
    {
        ...
        Kitty storage sire = kitties[_sireId];
        return _isValidMatingPair(maobc, _maobcId, sire, _sireId) &&
            _isSiringPermitted(_sireId, _maobcId);
    }

    /// @dev Internal utility function to initiate breeding, assumes that all breeding
    ///  requirements have been checked.
    function _breedWith(uint256 _maobcId, uint256 _sireId) internal {
        // Grab a reference to the Kitties from storage.
        Kitty storage sire = kitties[_sireId];
        Kitty storage maobc = kitties[_maobcId];

        // Mark the maobc as pregnant, keeping track of who the sire is.
        maobc.siringWithId = uint32(_sireId);

        // Trigger the cooldown for both parents.
        _triggerCooldown(sire);
        _triggerCooldown(maobc);

        // Clear siring permission for both parents. This may not be strictly necessary
        // but it's likely to avoid confusion!
        delete sireAllowedToAddress[_maobcId];
        ...
        emit Pregnant(kittyIndexToOwner[_maobcId], _maobcId, _sireId, maobc.cooldownEndBlock);
    }

    /// @notice Breed a Kitty you own (as maobc) with a sire that you own, or for which you
    ///  have previously been given Siring approval. Will either make your cat pregnant, or will
    ///  fail entirely. Requires a pre-payment of the fee given out to the first caller of giveBirth()
    /// @param _maobcId The ID of the Kitty acting as maobc (will end up pregnant if successful)
    /// @param _sireId The ID of the Kitty acting as sire (will begin its siring cooldown if successful)
    function breedWithAuto(uint256 _maobcId, uint256 _sireId)
        external
        payable
        whenNotPaused
    {
        ...
        // Caller must own the maobc.
        require(_owns(msg.sender, _maobcId));

        // Neither sire nor maobc are allowed to be on auction during a normal
        // breeding operation, but we don't need to check that explicitly.
        // For maobc: The caller of this function can't be the owner of the maobc
        ...
        // Check that maobc and sire are both owned by caller, or that the sire
        // has given siring permission to caller (i.e. maobc's owner).
        // Will fail for _sireId = 0
        require(_isSiringPermitted(_sireId, _maobcId));

        // Grab a reference to the potential maobc
        Kitty storage maobc = kitties[_maobcId];

        // Make sure maobc isn't pregnant, or in the middle of a siring cooldown
        require(_isReadyToBreed(maobc));
        ...
        _breedWith(_maobcId, _sireId);
    }

    /// @notice Have a pregnant Kitty give birth!
    /// @param _maobcId A Kitty ready to give birth.
    ...
    ///  to the current owner of the maobc. Upon successful completion, both the maobc and the
    ///  new kitten will be ready to breed again. Note that anyone can call this function (if they
    ///  are willing to pay the gas!), but the new kitten always goes to the mother's owner.
    function giveBirth(uint256 _maobcId)
        external
        whenNotPaused
        returns(uint256)
    {
        // Grab a reference to the maobc in storage.
        Kitty storage maobc = kitties[_maobcId];

        // Check that the maobc is a valid cat.
        require(maobc.birthTime != 0);

        // Check that the maobc is pregnant, and that its time has come!
        require(_isReadyToGiveBirth(maobc));

        // Grab a reference to the sire in storage.
        uint256 sireId = maobc.siringWithId;
        Kitty storage sire = kitties[sireId];

        // Determine the higher generation number of the two parents
        uint16 parentGen = maobc.generation;
        if (sire.generation > maobc.generation) {
            parentGen = sire.generation;
        }

        // Call the sooper-sekret gene mixing operation.
        uint256 childGenes = geneScience.mixGenes(maobc.genes, sire.genes, maobc.cooldownEndBlock - 1);

        // Make the new kitten!
        address owner = kittyIndexToOwner[_maobcId];
        uint256 kittenId = _createKitty(_maobcId, maobc.siringWithId, parentGen + 1, childGenes, owner);

        // Clear the reference to sire from the maobc (REQUIRED! Having siringWithId
        // set is what marks a maobc as being pregnant.)
        delete maobc.siringWithId;
...
contract KittyAuction is KittyBreeding {
    ...
    ///  Immediately breeds the winning maobc with the sire on auction.
    /// @param _sireId - ID of the sire on auction.
    /// @param _maobcId - ID of the maobc owned by the bidder.
        ...
        whenNotPaused
    {
        // Auction contract checks input sizes
        require(_owns(msg.sender, _maobcId));
        require(isReadyToBreed(_maobcId));
        require(_canBreedWithViaAuction(_maobcId, _sireId));
        ...
        _breedWith(uint32(_maobcId), uint32(_sireId));
    }
