...
import org.obc.core.vm.repository.Type;
...
public class DepositImpl implements Deposit {
  ...
  public synchronized Storage getStorage(byte[] address) {
    ...
    ContractCapsule contract = getContract(address);
    if (contract != null && !ByteUtil.isNullOrZeroArray(contract.getobcHash())) {
      storage.generateAddrHash(contract.getobcHash());
    }
    return storage;
  }
  ...
  public synchronized void putStorageValue(byte[] address, DataWord key, DataWord value) {
    address = convertToobcAddress(address);
  ...
  @Override
  public TransactionCapsule getTransaction(byte[] obcHash) {
    Key key = Key.create(obcHash);
    ...
    if (parent != null) {
      transactionCapsule = parent.getTransaction(obcHash);
    } else {
      try {
        transactionCapsule = getTransactionStore().get(obcHash);
