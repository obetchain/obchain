...
import static org.obc.protos.Protocol.Transaction.Contract.ContractType.ShieldedTransferContract;
...
import org.obc.core.db.ResourceProcessor;
...
public class BandwidthProcessor extends ResourceProcessor {
  ...
  @Override
  public void consume(TransactionCapsule obc, TransactionTrace trace)
      throws ContractValidateException, AccountResourceInsufficientException,
      TooBigTransactionResultException {
    List<Contract> contracts = obc.getInstance().getRawData().getContractList();
    if (obc.getResultSerializedSize() > Constant.MAX_RESULT_SIZE_IN_TX * contracts.size()) {
    ...
    if (chainBaseManager.getDynamicPropertiesStore().supportVM()) {
      bytesSize = obc.getInstance().toBuilder().clearRet().build().getSerializedSize();
    } else {
      bytesSize = obc.getSerializedSize();
    }
    ...
    for (Contract contract : contracts) {
      ...
      logger.debug("obcId {}, bandwidth cost: {}", obc.getTransactionId(), bytesSize);
  ...
  public long calculateGlobalNetLimit(AccountCapsule accountCapsule) {
    ...
    long netWeight = frozeBalance / obc_PRECISION;
