...
import static org.obc.common.utils.Commons.decodeFromBase58Check;
...
public class Util {
  ...
  public static byte[] generateContractAddress(Transaction obc, byte[] ownerAddress) {
    // get tx hash
    byte[] txRawDataHash = Sha256Hash
        .of(CommonParameter.getInstance().isECKeyCryptoEngine(), obc.getRawData().toByteArray())
        .getBytes();
  ...
  public static List<Log> convertLogAddressToobcAddress(TransactionInfo transactionInfo) {
    ...
    for (Log log : transactionInfo.getLogList()) {
      ...
      if (oldAddress.length == 0 || oldAddress.length > 20) {
        ...
        logBuilder
            .setAddress(ByteString.copyFrom(TransactionTrace.convertToobcAddress(newAddress)));
      }
