...
import org.obc.core.vm.PrecompiledContracts.BatchValidateSign;
...
public class BatchValidateSignContractTest {
  ...
  void staticCallTest() {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      if (i == 13) {
        addresses
            .add(StringUtil.encode58Check(TransactionTrace.convertToobcAddress(new byte[20])));
      } else {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      if (i % 5 == 0) {
        addresses.add(StringUtil.encode58Check(TransactionTrace
            .convertToobcAddress(new byte[20])));
        signatures.add(Hex.toHexString(DataWord.ONE().getData()));
      } else {
        addresses.add(StringUtil.encode58Check(key.getAddress()));
        signatures.add(Hex.toHexString(sign));
