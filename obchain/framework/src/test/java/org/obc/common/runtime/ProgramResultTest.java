...
import static org.obc.core.capsule.utils.TransactionUtil.buildTransactionInfoInstance;
import static org.obc.core.utils.TransactionUtil.generateContractAddress;
...
public class ProgramResultTest {
  ...
  public void successAndFailResultTest()
    ...
    TransactionTrace traceSuccess = TvmTestUtils
        .processTransactionAndReturnTrace(obc1, deposit, null);
    ...
    checkTransactionInfo(traceSuccess, obc1, null, internalTransactionsList);
    ...
    Transaction obc2 = TvmTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), aContract,
            triggerData2, 0, 100000000);
    TransactionTrace traceFailed = TvmTestUtils
        .processTransactionAndReturnTrace(obc2, deposit, null);
    runtime = traceFailed.getRuntime();

    byte[] bContract2 =
        generateContractAddress(new TransactionCapsule(obc2).getTransactionId().getBytes(), 0);
    ...
    checkTransactionInfo(traceFailed, obc2, null, internalTransactionsListFail);
  }
  ...
  public void suicideResultTest()
    ...
    Transaction obc = TvmTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), suicideContract,
            triggerData1, 0, 100000000);
    TransactionTrace trace = TvmTestUtils.processTransactionAndReturnTrace(obc, deposit, null);
    ...
    Assert.assertEquals(internalTransactionsList.get(0).isRejected(), false);
    checkTransactionInfo(trace, obc, null, internalTransactionsList);
  }
  ...
  public void checkTransactionInfo(TransactionTrace trace, Transaction obc, BlockCapsule block,
      List<InternalTransaction> internalTransactionsList) {
    TransactionInfoCapsule obcInfoCapsule =
        buildTransactionInfoInstance(new TransactionCapsule(obc), null, trace);
    List<Protocol.InternalTransaction> internalTransactionListFromProtocol = obcInfoCapsule
