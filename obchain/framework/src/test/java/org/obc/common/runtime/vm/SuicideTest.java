...
*/
  ...
  public void testSuicide() throws Exception {
    ...
    // deploy contract
    Protocol.Transaction obc = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, ownerAddress, abi, factoryCode, 100000000, feeLimit, 0,
        null);
    byte[] factoryAddress = WalletUtil.generateContractAddress(obc);
    ...
    obc = TvmTestUtils.generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS),
        factoryAddress, Hex.decode(hexInput), 0, feeLimit);
    InternalTransaction rootInternalTransaction = new InternalTransaction(obc,
        InternalTransaction.obcType.obc_CONTRACT_CALL_TYPE);
    repository = RepositoryImpl.createRoot(storeFactory);
    ProgramInvoke programInvoke = programInvokeFactory
        .createProgramInvoke(InternalTransaction.obcType.obc_CONTRACT_CALL_TYPE,
            InternalTransaction.ExecutorType.ET_PRE_TYPE, obc,
    ...
    obc = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, ownerAddress, abi, factoryCode, 100000000, feeLimit, 0,
        null);
    factoryAddress = WalletUtil.generateContractAddress(obc);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(obc, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
    ...
    obc = TvmTestUtils.generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS),
        factoryAddress, Hex.decode(hexInput), 0, feeLimit);
    rootInternalTransaction = new InternalTransaction(obc,
        InternalTransaction.obcType.obc_CONTRACT_CALL_TYPE);
    repository = RepositoryImpl.createRoot(storeFactory);
    ...
    obc = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] obtainContractAddr = WalletUtil.generateContractAddress(obc);
    ...
    //test vote
    final byte[] zeroAddr = TransactionTrace.convertToobcAddress(new byte[20]);
    //deploy contract1
    obc = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, ownerAddress, abi, factoryCode, 100000000, feeLimit, 0,
        null);
    ...
    obc = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        "contractObtain", ownerAddress, abi, factoryCode, 100000000, feeLimit, 0,
        null);
    obtainContractAddr = WalletUtil.generateContractAddress(obc);
    obtainContractAddrStr = StringUtil.encode58Check(obtainContractAddr);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(obc, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
    //deploy contract2
    obc = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] suicideContractAddr = WalletUtil.generateContractAddress(obc);
