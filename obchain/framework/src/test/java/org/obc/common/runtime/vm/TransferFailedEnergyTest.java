...
import org.obc.common.runtime.TVMTestResult;
...
public class TransferFailedEnergyTest extends VMTestBase {
  ...
  contract EnergyOfTransferFailedTest {
      ...
      // InsufficientBalance
      function testTransferobcInsufficientBalance() payable public{
          msg.sender.transfer(10);
      }

      function testSendobcInsufficientBalance() payable public{
          msg.sender.send(10);
      }
      ...
      function testCallobcInsufficientBalance(address payable caller) public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }

      function testCreateobcInsufficientBalance() payable public {
          (new Caller).value(10)();
      }

      // NonexistentTarget
      function testTransferobcNonexistentTarget(address payable nonexistentTarget) payable public {
      ...
      function testCallobcNonexistentTarget(address payable nonexistentTarget) payable public {
          require(address(this).balance >= 10);
      ...
      // target is self
      function testTransferobcSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.transfer(10);
      }

      function testSendobcSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.send(10);
      }
  ...
  contract EnergyOfTransferFailedTest {
      ...
      function testCallobcInsufficientBalance(address caller) payable public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }
      ...
      // NonexistentTarget
      function testTransferobcNonexistentTarget(address nonexistentTarget) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transfer(10);
      }
      ...
      function testCallobcNonexistentTarget(address nonexistentTarget) public {
          require(address(this).balance >= 10);
          nonexistentTarget.call.value(10)(abi.encodeWithSignature("test()"));
      }
  ...
  */
  ...
  TestCase[] testCasesAfterAllowTvmConstantinop = {
      new TestCase("testTransferobcSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSendobcSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSuicideNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testTransferobcNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testCallobcNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
  };
  TestCase[] testCasesBeforeAllowTvmConstantinop = {
      new TestCase("testTransferobcSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSendobcSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSuicideNonexistentTarget(address)",
  ...
  TestCase[] testCasesInsufficientBalance = {
      new TestCase("testTransferobcInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testSendobcInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.SUCCESS),
      new TestCase("testCreateobcInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testCallobcInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
  ...
  public void testTransferFailedAfterAllowTvmConstantinopl()
    ...
    // deploy contract
    Transaction obc = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, value, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithSufficientBalance = WalletUtil.generateContractAddress(obc);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(obc, rootDeposit, null);
    ...
    obc = TvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, 0, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithoutBalance = WalletUtil.generateContractAddress(obc);
    runtime = TvmTestUtils.processTransactionAndReturnRuntime(obc, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
