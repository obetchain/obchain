package org.obc.common.overlay.discover.node;
...
import org.obc.common.overlay.discover.node.statistics.MessageStatistics;
import org.obc.common.overlay.discover.node.statistics.NodeStatistics;
import org.obc.common.overlay.message.DisconnectMessage;
import org.obc.common.overlay.message.PongMessage;
...
public class NodeStatisticsTest {
  ...
  public void testMessage() {
    ...
    Assert.assertEquals(1, statistics.obcInSyncBlockChain.getTotalCount());
    ...
    Assert.assertEquals(1, statistics.obcOutBlockChainInventory.getTotalCount());

    InventoryMessage invMsgobc =
        new InventoryMessage(new ArrayList<>(), Protocol.Inventory.InventoryType.obc);
    String inventoryMessageStr = invMsgobc.toString();
    Assert.assertNotNull(inventoryMessageStr);
    statistics.addTcpInMessage(invMsgobc);
    statistics.addTcpOutMessage(invMsgobc);
    ...
    Assert.assertEquals(1, statistics.obcInBlockInventory.getTotalCount());

    FetchInvDataMessage fetchInvDataobc =
        new FetchInvDataMessage(new ArrayList<>(), Protocol.Inventory.InventoryType.obc);
    statistics.addTcpInMessage(fetchInvDataobc);
    statistics.addTcpOutMessage(fetchInvDataobc);
    ...
    Assert.assertEquals(1, statistics.obcInobcFetchInvData.getTotalCount());
    ...
    Assert.assertEquals(1, statistics.obcInobcs.getTotalCount());
    ...
    long inBlockCount = statistics.obcInBlock.getTotalCount();
