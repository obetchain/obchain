...
public class RevokingDbWithCacheNewValueTest {
  ...
  public synchronized void testPop() throws RevokingStoreIllegalStateException {
    ...
    obcDatabase = new TestRevokingobcStore("testRevokingDBWithCacheNewValue-testPop");
  ...
  public synchronized void testMerge() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    obcDatabase = new TestRevokingobcStore("testRevokingDBWithCacheNewValue-testMerge");
    revokingDatabase.add(obcDatabase.getRevokingDB());
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(testProtoCapsule.getData(), testProtoCapsule2);
      tmpSession.merge();
    }
    Assert.assertEquals(testProtoCapsule2, obcDatabase.get(testProtoCapsule.getData()));

    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.delete(testProtoCapsule.getData());
      tmpSession.merge();
    }
    Assert.assertNull(obcDatabase.get(testProtoCapsule.getData()));
    dialog.reset();
  }
  ...
  public synchronized void testRevoke() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    obcDatabase = new TestRevokingobcStore("testRevokingDBWithCacheNewValue-testRevoke");
    revokingDatabase.add(obcDatabase.getRevokingDB());
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(testProtoCapsule.getData(), new ProtoCapsuleTest("revoke22".getBytes()));
      tmpSession.merge();
    }

    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(testProtoCapsule.getData(), new ProtoCapsuleTest("revoke222".getBytes()));
      tmpSession.merge();
    }
    ...
    logger.info(
        "**********testProtoCapsule:" + (obcDatabase.getUnchecked(testProtoCapsule.getData()))
            .toString());
    Assert.assertEquals(testProtoCapsule, obcDatabase.get(testProtoCapsule.getData()));
  }
  ...
  public synchronized void testGetlatestValues() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    obcDatabase = new TestRevokingobcStore("testSnapshotManager-testGetlatestValues");
    revokingDatabase.add(obcDatabase.getRevokingDB());
    while (revokingDatabase.size() != 0) {
      revokingDatabase.pop();
    }
  ...
  public synchronized void testGetValuesNext() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    obcDatabase = new TestRevokingobcStore("testSnapshotManager-testGetValuesNext");
    revokingDatabase.add(obcDatabase.getRevokingDB());
    while (revokingDatabase.size() != 0) {
      revokingDatabase.pop();
    }
  ...
  public synchronized void testGetKeysNext() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    obcDatabase = new TestRevokingobcStore("testSnapshotManager-testGetKeysNext");
    revokingDatabase.add(obcDatabase.getRevokingDB());
    while (revokingDatabase.size() != 0) {
      revokingDatabase.pop();
    }
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(pairPriceKey2, testProtoCapsule);
      tmpSession.commit();
    }
    testProtoCapsule = new ProtoCapsuleTest(("getKeysNext1").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(pairPriceKey1, testProtoCapsule);
      tmpSession.commit();
    }

    testProtoCapsule = new ProtoCapsuleTest(("getKeysNext0").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(pairPriceKey0, testProtoCapsule);
      tmpSession.commit();
    }
    testProtoCapsule = new ProtoCapsuleTest(("getKeysNext3").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(pairPriceKey3, testProtoCapsule);
      tmpSession.commit();
    }

    List<byte[]> result = obcDatabase.getRevokingDB().getKeysNext(pairPriceKey0, 4);
  ...
  public synchronized void testGetKeysNextWithSameKey() {
    revokingDatabase = context.getBean(SnapshotManager.class);
    revokingDatabase.enable();
    obcDatabase = new TestRevokingobcStore("testSnapshotManager-testGetKeysNextWithSameKey");
    revokingDatabase.add(obcDatabase.getRevokingDB());
    while (revokingDatabase.size() != 0) {
      revokingDatabase.pop();
    }
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(pairPriceKey2, testProtoCapsule2);
      tmpSession.commit();
    }
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(pairPriceKey1, testProtoCapsule1);
      tmpSession.commit();
    }
    ...
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(pairPriceKey0, testProtoCapsule0);
      tmpSession.commit();
    }

    ProtoCapsuleTest testProtoCapsule3 = new ProtoCapsuleTest(("getKeysNext3").getBytes());
    try (ISession tmpSession = revokingDatabase.buildSession()) {
      obcDatabase.put(pairPriceKey3, testProtoCapsule3);
      tmpSession.commit();
    }

    List<byte[]> result = obcDatabase.getRevokingDB().getKeysNext(pairPriceKey0, 3);
  ...
  public static class TestRevokingobcStore extends obcStoreWithRevoking<ProtoCapsuleTest> {

    protected TestRevokingobcStore(String dbName) {
      super(dbName);
    }
