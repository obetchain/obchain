...
import org.obc.core.actuator.TransferActuator;
...
public class TransferActuatorTest {
  ...
  public void iniviateobc() {
    ...
    try {
      actuator.validate();
      actuator.execute(ret);
      fail("Cannot transfer obc to yourself.");

    } catch (ContractValidateException e) {
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("Cannot transfer obc to yourself.", e.getMessage());
      AccountCapsule owner = dbManager.getAccountStore()
          .get(ByteArray.fromHexString(OWNER_ADDRESS));
      AccountCapsule toAccount = dbManager.getAccountStore()
          .get(ByteArray.fromHexString(TO_ADDRESS));
