...
public class TransactionCapsuleTest {
  ...
  public void addSign() {
    ...
    Transaction.Builder obcBuilder = Transaction.newBuilder();
    ...
    obcBuilder.setRawData(rawBuilder);
    AccountStore accountStore = dbManager.getAccountStore();
    TransactionCapsule transactionCapsule = new TransactionCapsule(obcBuilder.build());
    //Accout not exist
    try {
  ...
  public void validateSignature0() {
    ...
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    obcBuilder.clearSignature().addSignature(test);
    try {
      TransactionCapsule.validateSignature(obcBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    new Random().nextBytes(rand);
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    obcBuilder.clearSignature().addSignature(ByteString.copyFrom(rand));
    try {
      TransactionCapsule.validateSignature(obcBuilder.build(), hash,
          accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertEquals(e.getMessage(), "Header byte out of range: 35");
    ...
    obcBuilder.clearSignature().addAllSignature(sign21_11);
    try {
      TransactionCapsule.validateSignature(obcBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
    ...
    obcBuilder.clearSignature().addAllSignature(sign21_11_22_23);
    try {
      TransactionCapsule
          .validateSignature(obcBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    obcBuilder.clearSignature().addAllSignature(sign21_22_21);
    try {
      TransactionCapsule
          .validateSignature(obcBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    ...
    obcBuilder.clearSignature().addAllSignature(sign21);
    try {
    ...
    obcBuilder.clearSignature().addAllSignature(sign21_22);
    try {
      boolean result = TransactionCapsule
          .validateSignature(obcBuilder.build(), hash, accountStore);
    ...
    obcBuilder.clearSignature().addAllSignature(sign21_22_23);
    try {
      boolean result = TransactionCapsule
          .validateSignature(obcBuilder.build(), hash, accountStore);
      Assert.assertTrue(result);
  ...
  public void validateSignature1() {
    ...
    ArrayList<ByteString> sign = sign(prikeys, Sha256Hash.hash(rawBuilder.build().toByteArray()));
    obcBuilder.addAllSignature(sign);
    transactionCapsule = new TransactionCapsule(obcBuilder.build());

    // no contract
    prikeys.clear();
    prikeys.add(ByteArray.fromHexString(KEY_21));
    obcBuilder = Transaction.newBuilder();
    ...
    obcBuilder = transactionCapsule.getInstance().toBuilder();
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    obcBuilder.clearSignature();
    obcBuilder.addSignature(test);
    transactionCapsule = new TransactionCapsule(obcBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    obcBuilder.clearSignature();
    obcBuilder.addSignature(ByteString.copyFrom(rand));
    transactionCapsule = new TransactionCapsule(obcBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11 = sign(prikeys, hash);
    obcBuilder.clearSignature();
    obcBuilder.addAllSignature(sign21_11);
    transactionCapsule = new TransactionCapsule(obcBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11_22_23 = sign(prikeys, hash);
    obcBuilder.clearSignature();
    obcBuilder.addAllSignature(sign21_11_22_23);
    transactionCapsule = new TransactionCapsule(obcBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22_21 = sign(prikeys, hash);
    obcBuilder.clearSignature();
    obcBuilder.addAllSignature(sign21_22_21);
    transactionCapsule = new TransactionCapsule(obcBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21 = sign(prikeys, hash);
    obcBuilder.clearSignature();
    obcBuilder.addAllSignature(sign21);
    transactionCapsule = new TransactionCapsule(obcBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22 = sign(prikeys, hash);
    obcBuilder.clearSignature();
    obcBuilder.addAllSignature(sign21_22);
    transactionCapsule = new TransactionCapsule(obcBuilder.build());
    try {
    ...
    ArrayList<ByteString> sign21_22_23 = sign(prikeys, hash);
    obcBuilder.clearSignature();
    obcBuilder.addAllSignature(sign21_22_23);
    transactionCapsule = new TransactionCapsule(obcBuilder.build());
    try {
      boolean result = transactionCapsule.validateSignature(dbManager);
      Assert.assertTrue(result);
  ...
  @Test
  public void obcCapsuleClearTest() {
    Transaction tx = Transaction.newBuilder()
        .addRet(Result.newBuilder().setContractRet(contractResult.OUT_OF_TIME).build()).build();
    TransactionCapsule obcCap = new TransactionCapsule(tx);
    Result.contractResult contractResult = obcCap.getContractResult();
    obcCap.resetResult();
    Assert.assertEquals(obcCap.getInstance().getRetCount(), 0);
    obcCap.setResultCode(contractResult);
    Assert.assertEquals(obcCap.getInstance()
