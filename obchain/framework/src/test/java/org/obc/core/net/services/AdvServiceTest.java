package org.obc.core.net.services;
...
public class AdvServiceTest {
  ...
  private void testAddInv() {
    boolean flag;
    Item itemobc = new Item(Sha256Hash.ZERO_HASH, InventoryType.obc);
    flag = service.addInv(itemobc);
  ...
  private void testobcBroadcast() {
    Protocol.Transaction obc = Protocol.Transaction.newBuilder().build();
    CommonParameter.getInstance().setValidContractProtoThreadNum(1);
    TransactionMessage msg = new TransactionMessage(obc);
    service.broadcast(msg);
    Item item = new Item(msg.getMessageId(), InventoryType.obc);
    Assert.assertNotNull(service.getMessage(item));
  }
